...
//         m_func_options("breakpoint set", false, 'F') {
//     // SetHelpLong(
//     //   "ggg");
...
public:
  ...
  CommandObjectBreakpointSetTenX(CommandInterpreter &interpreter)
      : CommandObjectParsed(
            interpreter, "breakpoint set10x",
            "Sets a breakpoint or set of breakpoints in the executable.",
            "breakpoint set <cmd-options>"),
        m_python_class_options("scripted breakpoint", true, 'P') {
    // We're picking up all the normal options, commands and disable.
  ...
  ~CommandObjectBreakpointSetTenX() override = default;
...
protected:
  bool DoExecute(Args &command, CommandReturnObject &result) override {
    ...
    printf("command.GetArgumentAtIndex(0): %s\n", command.GetArgumentAtIndex(0));
    printf("command.GetArgumentAtIndex(1): %s\n", command.GetArgumentAtIndex(1));
  ...
  //           command.GetArgumentAtIndex(0) == "set10x" &&
  //           command.GetArgumentAtIndex(1) == "hello") {

  //           printf("Yes Thats your command :)");
  ...
  //       }
    ...
    case eSetTypeMain: // Breakpoint by function name
    {
      FunctionNameType name_type_mask = m_options.m_func_name_type_mask;
    ...
    if (bp_sp) {
    // Add the "bt" command to the breakpoint options
    auto cmd_data = std::make_unique<BreakpointOptions::CommandData>();
    cmd_data->user_source.AppendString("bt"); // Hardcoded 'bt' command
...
CommandObjectMultiwordBreakpoint::CommandObjectMultiwordBreakpoint(
  ...
  CommandObjectSP setTenX_command_object(
      new CommandObjectBreakpointSetTenX(interpreter));
  ...
  setTenX_command_object->SetCommandName("breakpoint set10x");
  ...
  LoadSubCommand("set10x", setTenX_command_object);
